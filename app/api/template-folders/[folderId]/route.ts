...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    if (!folder) {
      return NextResponse.json({ error: "Template folder was not found." }, { status: 404 });
    }
    ...
    if ((count || 0) > 0) {
      return NextResponse.json({ error: "Move or delete the templates inside this folder before deleting it." }, { status: 409 });
    }
    ...
    return NextResponse.json({ error: getTemplateFolderError(error, "Template folder could not be deleted.") }, { status: 500 });
  }
}
